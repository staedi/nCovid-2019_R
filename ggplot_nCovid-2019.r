...
merge_data <- function(cutoff=60,allow_minus=FALSE,remote='N') {
  ...
  covid <<- covid %>%
    ...
    dplyr::mutate(Confirmed = dplyr::if_else(merge_type=='country',`Total Confirmed`,Confirmed),
                  ...
                  rConfirmed = dplyr::if_else(merge_type=='country',rTConfirmed,rConfirmed),
                  rDeaths = dplyr::if_else(merge_type=='country',rTDeaths,rDeaths),
    ...
    dplyr::group_by(adm0_a3,`Country/Region`,`Province/State`,merge_type,Date,
                    Confirmed,Deaths,`Total Confirmed`,`Total Deaths`,
                    i_Confirmed,i_Deaths,iTot_Confirmed,iTot_Deaths,
                    rConfirmed,rDeaths,rTConfirmed,rTDeaths,
    ...
    for (cty in unique(cand$adm0_a3)) {
      ...
      for (iter in 1:2) {
        if (iter==1) {
          ...
          # target_text <- 'infections'
          total_cty <- max(tail(covid[covid$adm0_a3 == cty,'Total Confirmed'],1))
          ...
          # target_col <- 'i_Deaths'
          # target_text <- 'casualties'
          total_cty <- max(tail(covid[covid$adm0_a3 == cty,'Total Deaths'],1))
